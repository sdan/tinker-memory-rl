"""
SFT training for Q2: supervised memorization baseline.

Thin wrapper around the standard supervised training loop that points to the
memory-specific dataset created by ``sft_data_gen.py`` and reuses the RL
environments for evaluation.
"""

import asyncio
import math
from datetime import datetime
from pathlib import Path
from typing import Literal

import chz
import tinker

from tinker_cookbook import cli_utils, model_info
from tinker_cookbook.eval.evaluators import EvaluatorBuilder, SamplingClientEvaluator
from tinker_cookbook.recipes.memory_rl.multi_step_env import MultiStepDatasetBuilder
from tinker_cookbook.recipes.memory_rl.single_step_env import SingleStepDatasetBuilder
from tinker_cookbook.recipes.memory_rl.task_utils import RewardType
from tinker_cookbook.renderers import TrainOnWhat
from tinker_cookbook.rl.metric_util import RLTestSetEvaluator
from tinker_cookbook.supervised import train
from tinker_cookbook.supervised.data import FromConversationFileBuilder
from tinker_cookbook.supervised.nll_evaluator import NLLEvaluator
from tinker_cookbook.supervised.types import ChatDatasetBuilderCommonConfig


@chz.chz
class SFTTrainConfig:
    """
    CLI configuration for SFT training on the memory task.

    This mirrors the RL config in ``rl_train.py`` but points to a JSONL
    conversations file generated by ``sft_data_gen.py``.
    """

    # Model
    model_name: str = "meta-llama/Llama-3.1-8B"
    renderer_name: str | None = None

    # Data (paths should match ``sft_data_gen.py`` defaults)
    N: int = 16  # For logging only; SFT data already encodes N in metadata.
    train_data_path: str = "/tmp/tinker-examples/memory_rl/sft_data/train.jsonl"
    test_data_path: str | None = "/tmp/tinker-examples/memory_rl/sft_data/test.jsonl"
    fixed_secret: int | None = None
    reward_type: RewardType = "binary"

    # Training
    learning_rate: float = 2e-4
    lr_schedule: str = "linear"
    num_epochs: int = 1
    batch_size: int = 128
    eval_every: int = 1

    # Environment evaluation (reuse RL envs as testbeds)
    eval_env_type: Literal["single_step", "multi_step"] = "single_step"
    eval_batch_size: int = 64
    eval_n_batches: int = 10
    eval_dataset_seed: int = 0
    eval_max_tokens: int = 8

    # Logging
    log_path: str | None = None
    wandb_project: str | None = None
    wandb_name: str | None = None

    # Service
    base_url: str | None = None
    max_length: int | None = 2048
    shuffle_seed: int = 0
    load_checkpoint_path: str | None = None


class _LazyEnvEvaluator(SamplingClientEvaluator):
    """Wrap RLTestSetEvaluator so SFT can re-use the RL envs for evaluation."""

    def __init__(
        self,
        dataset_builder: SingleStepDatasetBuilder | MultiStepDatasetBuilder,
        max_tokens: int,
        name: str = "env",
    ):
        self._dataset_builder = dataset_builder
        self._max_tokens = max_tokens
        self._name = name
        self._evaluator: RLTestSetEvaluator | None = None

    async def _ensure_evaluator(self) -> RLTestSetEvaluator:
        if self._evaluator is None:
            _, test_dataset = await self._dataset_builder()
            self._evaluator = RLTestSetEvaluator(
                dataset=test_dataset,
                max_tokens=self._max_tokens,
                name=self._name,
            )
        return self._evaluator

    async def __call__(self, sampling_client: tinker.SamplingClient) -> dict[str, float]:
        evaluator = await self._ensure_evaluator()
        return await evaluator(sampling_client)


def build_config(cli: SFTTrainConfig) -> train.Config:
    """Build supervised training config from CLI args."""

    # Validate reward type for single-step env evaluation consistency
    if cli.reward_type not in {"binary", "normalized_distance"}:
        raise ValueError(
            f"Unsupported reward_type '{cli.reward_type}'. Use 'binary' or 'normalized_distance'."
        )

    renderer_name = cli.renderer_name or model_info.get_recommended_renderer_name(cli.model_name)
    common_config = ChatDatasetBuilderCommonConfig(
        model_name_for_tokenizer=cli.model_name,
        renderer_name=renderer_name,
        max_length=cli.max_length,
        batch_size=cli.batch_size,
        train_on_what=TrainOnWhat.LAST_ASSISTANT_MESSAGE,
    )

    dataset_builder = FromConversationFileBuilder(
        common_config=common_config,
        file_path=cli.train_data_path,
        test_size=0,
        shuffle_seed=cli.shuffle_seed,
    )

    evaluator_builders: list[EvaluatorBuilder] = []

    # RL-style environment evaluation
    if cli.eval_env_type == "single_step":
        env_dataset_builder = SingleStepDatasetBuilder(
            batch_size=cli.eval_batch_size,
            model_name_for_tokenizer=cli.model_name,
            renderer_name=renderer_name,
            N=cli.N,
            reward_type=cli.reward_type,
            n_batches=cli.eval_n_batches,
            convo_prefix=None,
            fixed_secret=cli.fixed_secret,
            seed=cli.eval_dataset_seed,
        )
    elif cli.eval_env_type == "multi_step":
        env_dataset_builder = MultiStepDatasetBuilder(
            batch_size=cli.eval_batch_size,
            model_name_for_tokenizer=cli.model_name,
            renderer_name=renderer_name,
            N=cli.N,
            n_batches=cli.eval_n_batches,
            convo_prefix=None,
            fixed_secret=cli.fixed_secret,
            seed=cli.eval_dataset_seed,
        )
    else:
        raise ValueError(f"Unknown eval_env_type: {cli.eval_env_type}")

    evaluator_builders.append(
        lambda builder=env_dataset_builder: _LazyEnvEvaluator(
            dataset_builder=builder,
            max_tokens=cli.eval_max_tokens,
            name="env",
        )
    )

    # Optional NLL eval on held-out SFT data
    if cli.test_data_path:
        eval_builder = FromConversationFileBuilder(
            common_config=common_config,
            file_path=cli.test_data_path,
            test_size=0,
            shuffle_seed=cli.shuffle_seed,
        )
        eval_dataset, _ = eval_builder()

        def evaluator_factory(dataset=eval_dataset):
            return NLLEvaluator.from_dataset(dataset)

        evaluator_builders.append(evaluator_factory)

    date_and_time = datetime.now().strftime("%Y%m%d-%H%M%S")
    default_run_name = f"sft_N{cli.N}_bs{cli.batch_size}_lr{cli.learning_rate}_{date_and_time}"
    log_path = cli.log_path or f"/tmp/tinker-examples/memory_rl/sft/{default_run_name}"
    wandb_name = cli.wandb_name or default_run_name

    return train.Config(
        model_name=cli.model_name,
        load_checkpoint_path=cli.load_checkpoint_path,
        log_path=log_path,
        dataset_builder=dataset_builder,
        learning_rate=cli.learning_rate,
        lr_schedule=cli.lr_schedule,
        num_epochs=cli.num_epochs,
        eval_every=cli.eval_every,
        base_url=cli.base_url,
        evaluator_builders=evaluator_builders,
        wandb_project=cli.wandb_project,
        wandb_name=wandb_name,
    )


async def main() -> None:
    """CLI entrypoint for supervised memory training."""
    cli = chz.entrypoint(SFTTrainConfig)
    config = build_config(cli)

    # Print run summary
    entropy_bits = math.log2(cli.N) if cli.N > 0 else 0.0
    print(f"\n{'=' * 80}")
    print("SFT Memory Study - Run Configuration")
    print(f"{'=' * 80}")
    print(f"N (secret space): {cli.N} ({entropy_bits:.2f} bits entropy)")
    print(f"Fixed secret: {cli.fixed_secret if cli.fixed_secret is not None else 'Random'}")
    print(f"Training data: {cli.train_data_path}")
    if cli.test_data_path:
        print(f"Test data: {cli.test_data_path}")
    print(f"Eval env: {cli.eval_env_type} (reward_type: {cli.reward_type})")
    print(f"Epochs: {cli.num_epochs}, Batch size: {cli.batch_size}")
    print(f"Log path: {config.log_path}")
    print(f"{'=' * 80}\n")

    Path(config.log_path).parent.mkdir(parents=True, exist_ok=True)
    cli_utils.check_log_dir(config.log_path, behavior_if_exists="resume")
    await train.main(config)


if __name__ == "__main__":
    asyncio.run(main())

